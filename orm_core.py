...
if __name__ == '__main__':
    with engine.connect() as conn:
        ...
        # print(dir(result))
        print(f"Last ID: {jones_id}")

        ins_user = users.insert().values(fullname='Vasya Pupkin')
        result = conn.execute(ins_user)
        pupkin_id = result.lastrowid
        print(f"Last ID: {pupkin_id}")

        result = conn.execute(select(users))
        for row in result:
            print(row)

        ins_address = addresses.insert().values(email='jones@mail.com', user_id=jones_id)
        conn.execute(ins_address)
        ins_address = addresses.insert().values(email='pupkin@mail.com', user_id=pupkin_id)
        conn.execute(ins_address)
